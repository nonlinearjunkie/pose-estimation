...
def proposal_layer(inputs, proposal_count, nms_threshold, anchors, config=None):
    ...
    order = order[order < anchors.shape[0]]
    scores = scores[:order.shape[0]]
...
def refine_detections(rois, probs, deltas, window, config):
    ...
    if nms_keep is None:
        keep = torch.tensor([0, 0, 0, 0], dtype=torch.int64, device=rois.device)
    else:
...
class ViTBackbone(nn.Module):
    def __init__(self, config):
        ...
        self.vit = timm.create_model(
            ...
            pretrained=True,
            features_only=True,
            out_indices=config.OUT_INDICIES
            )
...
class MaskRCNNViT(nn.Module):
    ...
    def __init__(self, config, model_dir):
        ...
        """
        super(MaskRCNNViT, self).__init__()
        self.config = config
    ...
    def build(self, config):
        ...
        backbone = ViTBackbone(config)
        vit_dim = backbone.vit.feature_info.channels()[-1]
    ...
    def set_log_dir(self, model_path=None):
        ...
        self.checkpoint_path = os.path.join(self.log_dir, "vit_{}_*epoch*.pth".format(
    ...
    def train_model(self, train_dataset, val_dataset, learning_rate, epochs, layers):
        ...
        for epoch in range(self.epoch+1, epochs+1):
            ...
            # Save model
            if epoch == epochs or epoch==epochs-1:
                print("Saving model......")
